...
/* Cleanup handler to close file descriptor on early exit (interrupt/error) */
static void filelock__close_fd_cleanup(void *ptr)
{
  int *filedes = (int *)ptr;
  if (filedes && *filedes >= 0)
  {
    REprintf("Cleaning up: closing file descriptor %d\n", *filedes);
    close(*filedes);
    *filedes = -1;
  }
}

/* Structure to hold filelock_lock arguments and state */
struct filelock_lock_data
{
  const char *c_path;
  int c_exclusive;
  int c_timeout;
...
/* Worker function that will be called within cleanup context */
static SEXP filelock_lock_worker(void *data)
{
  struct filelock_lock_data *args = (struct filelock_lock_data *)data;
  struct flock lck;
  int ret;

  lck.l_type = args->c_exclusive ? F_WRLCK : F_RDLCK;
  lck.l_whence = SEEK_SET;
  lck.l_start = 0;
  lck.l_len = 0;

  args->filedes = open(args->c_path, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
  if (args->filedes == -1)
  {
    error("Cannot open lock file: %s", strerror(errno));
  }

  /* Register cleanup handler - will close filedes on interrupt/error */
  r_call_on_early_exit(filelock__close_fd_cleanup, &args->filedes);

  /* One shot only? Do not block if cannot lock */
  if (args->c_timeout == 0)
  {
    ret = fcntl(args->filedes, F_SETLK, &lck);
    if (ret == -1)
    {
      ...
      error("Cannot lock file: '%s': %s", args->c_path, strerror(errno));
    }
  }
  else
  {
    ret = filelock__interruptible(args->filedes, &lck, args->c_path,
                                  args->c_exclusive, args->c_timeout);
  }
  ...
  {
    return filelock__list_add(args->c_path, args->filedes, args->c_exclusive);
  }
}
...
SEXP filelock_lock(SEXP path, SEXP exclusive, SEXP timeout)
{
  ...
  /* Set up worker arguments and call with cleanup context */
  struct filelock_lock_data args = {
      .c_path = c_path,
      .c_exclusive = c_exclusive,
      .c_timeout = c_timeout,
      .filedes = -1};

  return r_with_cleanup_context(filelock_lock_worker, &args);
}
