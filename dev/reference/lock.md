...
- path:
  ...
  Path to the file to lock. If the file does not exist, it will be
  created, but the directory of the file must exist. *Do not place the
  lock on a file that you want to read from or write to!* \*Always use a
  special lock file. See details below.

- exclusive:

  Whether to acquire an exclusive lock. An exclusive lock gives the
  process exclusive access to the file, no other processes can place any
  kind of lock on it. A non-exclusive lock is a shared lock. Multiple
  processes can hold a shared lock on the same file. A process that
  writes to a file typically requests an exclusive lock, and a process
  that reads from it typically requests a shared lock.

- timeout:

  Timeout to acquire the lock in milliseconds. If `Inf`, then the
  process will wait indefinitely to acquire the lock. If zero, then the
  function it returns immediately, with or without acquiring the lock

- lock:

  The lock object to unlock. It is not an error to try to unlock an
  already unlocked lock. It is not possible to lock an unlocked lock
  again, a new lock has to be requested.

## Value

`lock` returns a `filelock_lock` object if the lock was successfully
acquired, and `NULL` if a timeout happened.

`unlock` returns `TRUE`, always.

## Warning

Always use special files for locking. I.e. if you want to restrict
access to a certain file, do *not* place the lock on this file. Create a
...
[`base::on.exit()`](https://rdrr.io/r/base/on.exit.html), so that the
